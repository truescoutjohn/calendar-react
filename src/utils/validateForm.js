...
const isDivideQuarterTimeInputs = inputs =>
  inputs.filter(input => input.split(TIME_SEPARATOR)[1] % QUARTER_HOUR !== 0).length === 0;
...
const isValidTimeInputs = (inputTime1, inputTime2) =>
  Math.abs(inputTime1.split(TIME_SEPARATOR)[0] - inputTime2.split(TIME_SEPARATOR)[0]) <=
  MAX_DIFFERENCE_HOURS_EVENTS;

export const isDeletable = (event, dataCurrentDate) => {
  const isPossibleDelete =
    (event.dateFrom >=
      new Date(dataCurrentDate.getTime() + QUARTER_HOUR * MILLISECONDS_IN_MINUTE) &&
      new Date(
